...
class AdvancedStockPredictor:
    """Hybrid LSTM-GRU stock predictor for 10-day prediction"""

    def __init__(self, sequence_length=60, prediction_days=10, features_to_use='all'):
    ...
    def prepare_data(self, df, target_column='Close'):
        ...
        # Fit scalersonly on training data
    ...
    def build_hybrid_model(self, input_shape, lstm_units=[128, 96], dropout=0.3, learning_rate=0.0008, seed=42):
        """
        Build Hybrid LSTM-GRU model for 10-day prediction
        Winner configuration: [128, 96] units
        """
        np.random.seed(seed)
    ...
    def train_single_model(self, X_train, y_train, X_val, y_val, epochs=150, batch_size=32,
                          lstm_units=[128, 96], dropout=0.3, learning_rate=0.0008, seed=42):
    ...
    def save_models_and_config(self, models_dir, ticker, metrics):
        ...
        """
        print("\n" + "="*70)
        print("SAVING MODELS AND CONFIGURATION FOR WEB DEPLOYMENT")
        print("="*70)
...
def generate_thesis_graphs(y_true, y_pred, ticker, graphs_dir, prediction_days=10):
    """
    Generate graphs
    """
    print("\n" + "="*70)
    ...
    # Predicted vs Actual Prices (for each prediction day)
    ...
    # Calculate residuals for Day 1 and Day 10
    residuals_day1 = y_true[:, 0] - y_pred[:, 0]
    residuals_day10 = y_true[:, -1] - y_pred[:, -1]
    ...
    actual_returns_day10 = np.cumsum((y_true[1:, -1] - y_true[:-1, -1]) / y_true[:-1, -1] * 100)
    pred_returns_day10 = np.cumsum((y_pred[1:, -1] - y_pred[:-1, -1]) / y_pred[:-1, -1] * 100)

    ax2 = axes[1]
    ax2.plot(actual_returns_day10, label='Actual Cumulative Returns', color='green', linewidth=2, alpha=0.7)
    ax2.plot(pred_returns_day10, label='Predicted Cumulative Returns', color='red', linewidth=2, linestyle='--', alpha=0.7)
...
def print_metrics_report(metrics, ticker):
    ...
    print("\nOVERALL PERFORMANCE (All 10 Days Combined):")
    ...
    # Highlight Day 10 specifically
    if 'Per_Day_Metrics' in metrics:
        day10 = metrics['Per_Day_Metrics']['Day_10']
        print("\n" + "="*70)
        print("DAY 10 PERFORMANCE (For Engineering Paper)")
        print("="*70)
        print(f"  • Day 10 MAE:   ${day10['MAE']:.4f}")
        print(f"  • Day 10 RMSE:  ${day10['RMSE']:.4f}")
        print(f"  • Day 10 MAPE:  {day10['MAPE']:.4f}%")
        print(f"  • Day 10 R²:    {day10['R2']:.4f}")
...
def main():
    ...
    print("\n" + "="*70)
    print("FINAL 10-DAY STOCK PREDICTION MODEL - ENHANCED FOR THESIS")
    print("Configuration: HYBRID_Larger_60d ([128, 96] units)")
    ...
    SEQUENCE_LENGTH = 60
    PREDICTION_DAYS = 10      # 10-day prediction
    EPOCHS = 150
    BATCH_SIZE = 32

    # Paths for saving
    MODELS_DIR = r'C:\Users\cinek\PycharmProjects\LSTMS\models\10d'
    GRAPHS_DIR = r'C:\Users\cinek\PycharmProjects\LSTMS\graphs\10d'
