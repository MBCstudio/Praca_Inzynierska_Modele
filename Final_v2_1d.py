...
def generate_thesis_graphs(y_true, y_pred, ticker, graphs_dir_5d, prediction_days=5):
    ...
    ax2.plot(y_pred[:, -1], label=f'Predicted Price (Day {prediction_days})', color='red', linewidth=2, linestyle='--',
             alpha=0.7)
    ax2.set_title(f'{ticker} - Day {prediction_days} Prediction: Actual vs Predicted Prices', fontsize=14,
                  fontweight='bold')
    ...
    ax2.plot(pred_returns_day5, label='Predicted Cumulative Returns', color='red', linewidth=2, linestyle='--',
             alpha=0.7)
...
def main():
    ...
    SEQUENCE_LENGTH = 40  # Best for 1d
    PREDICTION_DAYS = 5
    EPOCHS = 150
    BATCH_SIZE = 64
    ...
    predictor.save_models_and_config(MODELS_DIR_1D, TICKER, metrics)

    # Generate graphs
    print("\nStep 10: Generating thesis graphs for BOTH 1d and 5d...")
    generate_thesis_graphs(y_true, y_pred, TICKER, GRAPHS_DIR_1D, PREDICTION_DAYS)
